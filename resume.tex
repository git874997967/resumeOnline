...
\newRole{2019 -- present}
        {Software developer}
        {Microsoft}
        {Work as software engineer under Office IP antispam team. Response for maintaining and upgrading the ETL/ELT data pipeline, ADF jobs, and components in ADF and ADLS with Cosmos Scope and Kusto. As well as generate ad-hoc visuals with different tools based on different requirements. }
\roleAchievements{Migrated Cosmos uploader/downloader with ScopeSDK, TSQL and PowerShell from old JavaScriptlogic. Equipped with alert and pipeline monitor to enhance the durability.}
...
{Performed DB admin activities on the multiple DB Servers. Optimized and tuned slow running queries up to 30\% by troubleshooting issues with by using SQLServer Profiler and SQLServer Tuning Adviser. Improved the efficiency of the heavy-lift jobs/queries.}
